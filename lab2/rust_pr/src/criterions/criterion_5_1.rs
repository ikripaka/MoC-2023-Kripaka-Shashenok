...
use crate::internals::{
    calculate_probs, double_content, gen_affine_distortion, gen_random_n_l_grams,
    make_frequency_table, make_frequency_table_custom_manual, make_frequency_table_for_long_chunks,
    make_frequency_table_from_file, make_n_gram_on_content_from_str,
    recurrent_generation_n_l_grams, sort_hash_map_desc, vigenere_cipher_distortion,
};
...
pub fn run(filepath: &str) {
    ...
    let sorted_map_monogram = sort_hash_map_desc(&freq_table_monogram);
    let sorted_map_bigram = sort_hash_map_desc(&freq_table_bigram);
    ...
    for l_little in 1..=2 {
        ...
        if l_little as usize == L_MONOGRAM {
            (boxes1, boxes2, boxes3, boxes4) = (9, 9, 9, 9);
            (k_empty_1, k_empty_2, k_empty_3, k_empty_4) =
                (boxes1 - 5, boxes2 - 5, boxes3 - 5, boxes4 - 5);
        } else if l_little as usize == L_BIGRAM {
            (boxes1, boxes2, boxes3, boxes4) = (50, 50, 100, 200);
            (k_empty_1, k_empty_2, k_empty_3, k_empty_4) =
                (boxes1 - 5, boxes2 - 10, boxes3 - 60, boxes4 - 199);
        }

        rayon::scope(|s| {
            s.spawn(|_s| {
                res1_0 = criterion_5_1(
                    &sorted_maps,
                    &n_gram_l1,
                    boxes1,
                    k_empty_1,
...
fn sorting() {
    ...
    println!("{:?}", sort_hash_map_desc(&freq_table))
}
